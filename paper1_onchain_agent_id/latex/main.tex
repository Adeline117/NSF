...
\textbf{Important caveat.} Every held-out platform category is single-class (all-agent or all-human), which means the LOPO evaluation conflates platform behavioral novelty with label generalization. The 0.42 AUC reflects both genuine inability to learn transferable agent signatures AND the domain shift of encountering an entirely new behavioral profile. A mixed-class LOPO (where some platforms contribute both agents and humans) would isolate generalization more cleanly, but the dataset's provenance structure does not support this design. We therefore interpret Level~5 as measuring \emph{cross-context deployment readiness} rather than pure concept generalization.
