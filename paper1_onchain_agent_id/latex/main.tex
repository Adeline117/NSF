...
\begin{abstract}
We build a 23-feature behavioral classifier on 1,147 provenance-verified Ethereum addresses and subject it to five progressively honest evaluation levels---an \emph{honesty cascade}. At Level~1 (leaky cross-validation), AUC is 0.98. Fixing label leakage drops it to 0.80 (Level~2). A temporal holdout simulating forward deployment yields 0.67 (Level~3). A hand-curated 64-address sensitivity check gives 0.77 with wide confidence intervals (Level~4). Level~5 (leave-one-platform-out) yields a pooled AUC of 0.42---below chance---revealing that the classifier learns platform-specific behavioral signatures rather than a transferable agent concept. Because each platform contributes only one class, this conflates domain shift with concept generalization; the true cross-concept generalization gap likely lies between Level~2 (0.80) and Level~5 (0.42). Along the way, we discover that the widely assumed ``agents are riskier'' narrative is a labeling artifact: human DeFi users hold more unlimited token approvals (66.1 mean) than curated MEV bots (49.1 mean). We formalize a reusable three-step leakage-audit toolkit and release all code, features, labels, and diagnostic scripts. Our central contribution is the cascade itself: the progressive collapse from 0.98 to 0.42 reveals a gradient of evaluation realism, not a binary pass/fail. Within-platform detection works (Level~2, AUC~0.80); what does not yet exist is a platform-invariant feature set that transfers across operational contexts.
\end{abstract}

\keywords{on-chain AI agents, evaluation realism, honesty cascade, label leakage, cross-platform generalization, Ethereum, behavioral fingerprinting, domain shift}
\maketitle
...
5 & Leave-one-platform-out & \textbf{0.42} & Cross-context deployment readiness$^\dagger$ \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize $^\dagger$Single-class-per-platform confound: conflates domain shift with concept generalization (see Section~4.9).}
\end{table}
...
The Level~5 result is not noise. It means the classifier learns how Chainlink keepers differ from Gelato executors, not how agents differ from humans. Every AUC point between 0.98 and 0.42 was borrowed from an evaluation shortcut: label leakage, temporal overlap, or platform memorization. Strip them all away, and the task does not transfer---though it does work within the training distribution (Level~2, AUC~0.80). The research question is therefore not ``can we detect agents?'' (yes, within context) but ``can we build features that generalize across platforms?'' (not yet).
...
\begin{enumerate}
    ...
    \item \textbf{Within-platform detection works; cross-platform deployment does not transfer.} Within-platform detection achieves honest AUC~0.80 (Level~2); cross-platform leave-one-platform-out evaluation collapses to 0.42 (Level~5), establishing that current features capture platform-specific patterns, not platform-invariant agent signatures. The best-recognized category (Chainlink keepers, 68\% recall) and worst (DeFi high-frequency traders, 14\% recall) differ by 54 percentage points (Section~4.9).
...
\subsection{The Leakage Discovery}
...
\begin{enumerate}
    ...
    \item \textbf{Leave-one-platform-out evaluation (Level~5).} The first cross-platform generalization test for on-chain agent detection, showing pooled AUC~0.42 (below chance) across 14 held-out platform categories. Because each platform is single-class, this conflates domain shift with concept generalization; the result establishes that current features do not transfer across platforms, bounding true cross-platform generalization between Level~2 (0.80) and Level~5 (0.42).
...
\subsection{Bot Detection on Blockchains}

Detecting automated activity on blockchains has been studied primarily in the context of specific malicious behaviors. Chen et al.~\cite{chen2018ponzi} used transaction features and opcode analysis to detect Ponzi schemes on Ethereum, demonstrating that behavioral patterns in transaction data reveal automated activity. Victor and Weintraud~\cite{victor2021wash} detected wash trading on decentralized exchanges via graph features. Torres et al.~\cite{torres2021frontrunner} studied Ethereum frontrunner bots through mempool analysis, identifying displacement, insertion, and suppression attacks. Victor et al.~\cite{victor2023fingerprinting} proposed behavioral fingerprinting for blockchain bots using temporal and gas features similar in spirit to our temporal and gas groups, but restricted to MEV bots. Li et al.~\cite{li2024telegram} studied Telegram trading bots and their on-chain footprint.
...
\subsection{MEV and Automated Trading}

Daian et al.~\cite{daian2020flash} introduced MEV in ``Flash Boys 2.0'' and demonstrated that transaction ordering creates a searcher-bot ecosystem. Weintraub et al.~\cite{weintraub2022flashbot} measured Flashbots MEV at scale. Qin et al.~\cite{qin2022quantifying} quantified MEV across DeFi protocols, showing sandwich attacks, liquidations, and arbitrage extract billions annually. Park et al.~\cite{park2024mev} analyzed post-Merge MEV and the effect of Proposer-Builder Separation. Gupta et al.~\cite{gupta2023crossdomain} studied cross-domain MEV across chains and L2s.
...
\subsection{AI Agent Platforms}

Autonolas (OLAS)~\cite{autonolas2023} provides an open-source framework for autonomous agent services, with on-chain registration. AI16Z ELIZA~\cite{eliza2024} enables AI agents to interact with blockchain protocols through natural language. Virtuals Protocol~\cite{virtuals2024} offers a launchpad for tokenized AI agents. Fetch.ai~\cite{fetchai2023}, SingularityNET~\cite{singularitynet2023}, and AI Arena~\cite{aiarena2024} contribute to the growing on-chain agent population. These platforms deploy agents but do not provide cross-platform identification. A recent survey by He et al.~\cite{he2026agentsurvey} catalogs the taxonomy of on-chain AI agents but provides no empirical identification methodology.

\subsection{Blockchain Security Analysis}

Zhou et al.~\cite{zhou2023sok} systematized DeFi attacks; Wen et al.~\cite{wen2023composability} analyzed DeFi composability risk; Grech et al.~\cite{grech2022madmax} and Tsankov et al.~\cite{tsankov2018securify} built static analysis tools for smart contracts. Wang et al.~\cite{wang2024airisks} studied the risks of AI agents with cryptocurrency wallet access, identifying prompt injection as a primary attack vector. Zhang et al.~\cite{zhang2024approval} analyzed MaxUint256 approval attacks.
...
\subsection{Address Mining and Labeling}

\subsubsection{Strategy A: Platform Holders (C1-C4 labeled, \textbf{used for inference only})}
...
\subsubsection{Strategy B: Strict Provenance Subset (n=64, \textbf{sensitivity check})}
...
\subsubsection{Strategy C: Expanded Provenance Mining v4 (n=1,147, \textbf{primary honest set})}
...
\subsubsection{Decision: Demote C1-C4 to Evaluation}
...
\subsection{Feature Engineering: 23 Behavioral Features}
...
\subsection{Tabular Classifiers}
...
\subsection{Combined Paper 1 + Paper 3 Features}
...
\subsection{4-Dimensional Security Audit}
...
\section{Results: The Honesty Cascade}
...
\subsection{Honest Performance on Provenance-Verified Sets}

\subsubsection{Primary: Full Provenance Set (n=1,147, 533 agents, 614 humans)}
...
\subsubsection{Sensitivity Check: Strict Provenance Subset (n=64, 33 agents, 31 humans)}
...
\subsection{GNN vs Tabular Classifiers}
...
\subsection{SHAP Feature Explanations}
...
\subsection{Level 5: Leave-One-Platform-Out (The Cascade Floor)}
...
\textbf{Structural caveat (read before interpreting).} Every held-out platform category is single-class (all-agent or all-human), which means the LOPO evaluation conflates two distinct failure modes: (1)~genuine inability to learn transferable agent-vs-human signatures, and (2)~domain shift from encountering an entirely new behavioral profile. A mixed-class LOPO---where some platforms contribute both agents and humans---would isolate concept generalization from domain shift, but the dataset's provenance structure does not support this design. We therefore interpret the Level~5 results as measuring \emph{cross-context deployment readiness}: they establish a lower bound on generalization, not a precise estimate of concept failure. The true cross-concept generalization gap likely lies somewhere between Level~2 (0.80, within-platform) and Level~5 (reported below).

Because each category is single-class, per-platform AUC is undefined; we report per-platform accuracy (which equals recall for agent categories and specificity for human categories) and compute a \textbf{pooled AUC} over all 1{,}131 held-out predictions.
...
\begin{enumerate}
    \item \textbf{The pooled AUC is 0.42---below random chance.} A coin flip (AUC~0.50) would outperform the classifier under this protocol. Given the single-class-per-platform confound noted above, this measures the joint effect of concept failure and domain shift. What is clear: the classifier distinguishes Chainlink keepers from Gelato executors, not agents from humans in general.
    ...
    \item \textbf{The worst-recognized agent category is defi\_hf\_trader (14\% recall).} These 199 high-frequency Uniswap traders are the largest agent category and the most behaviorally distinctive. When held out, the classifier trained on Chainlink keepers, Gelato executors, and MEV bots does not recognize them. This indicates that ``high-frequency DeFi trading'' is a platform-specific behavioral mode that does not share enough signal with other agent categories for the current feature set to generalize.
...
\textbf{The cascade is now complete.} Level~1 (0.98) measured label recovery. Level~2 (0.80) measured within-platform pattern matching. Level~3 (0.67) measured same-era generalization. Level~4 (0.77) confirmed wide uncertainty. Level~5 (0.42) measured cross-context deployment readiness---and found that current features do not transfer. On-chain agent detection is a context-dependent capability: it works within the training distribution (Level~2) but does not generalize across platforms with current features.

\textbf{Recall the structural caveat above.} Because each platform is single-class, the 0.42 AUC conflates domain shift with concept failure. The classifier may partially understand the agent-vs-human distinction but fail to apply it when the behavioral distribution shifts. The honest summary is: on-chain agent detection is a \emph{context-dependent} capability---effective within the training distribution (Level~2, AUC~0.80), brittle outside it (Level~5, AUC~0.42). The research agenda is not ``can we detect agents?'' but ``can we build platform-invariant features?''
...
No prior blockchain bot-detection paper we are aware of reports a Level~5 evaluation. Without such a test, high-AUC numbers on pooled multi-source data cannot be distinguished from platform fingerprinting---though, as noted above, a mixed-class LOPO design would provide a cleaner test than the single-class-per-platform design we were constrained to use.


\section{Threats to Validity: The C1-C4 Label Leakage Discovery}
...
\subsection{A Reusable Leakage-Audit Toolkit}
...
\section{Discussion: What Does the Cascade Mean?}

\subsection{For Researchers}

The cascade from 0.98 to 0.42 has a clear methodological implication: \textbf{every published blockchain bot-detection result should include a cross-platform evaluation.} Without holding out entire platform categories, a reported AUC cannot be distinguished from platform fingerprinting. The field's standard practice---random cross-validation on pooled multi-source data---corresponds to Level~2 at best. Our Level~2-to-Level~5 gap (0.80 to 0.42) is an upper bound on the cross-platform penalty, because the single-class-per-platform confound inflates the apparent drop; the true generalization gap lies within this range but remains substantial.
...
\begin{itemize}
    ...
    \item \textbf{Cross-platform deployment does not transfer (Level~5).} At pooled AUC~0.42, the classifier produces anti-predictions on unseen platform categories under LOPO---though the single-class-per-platform design amplifies this failure. Regardless, the classifier should not be used to estimate the fraction of automated activity across the Ethereum ecosystem without retraining on each target platform's behavioral distribution.
...
The Level~5 result has a direct policy implication: \textbf{behavioral monitoring can distinguish AI agents from humans within known operational contexts (AUC~0.80) but does not yet transfer across platforms.} Any regulatory framework that depends on transaction-level behavioral classification to identify automated activity will work as a triage tool within its training distribution but will systematically miss novel agent categories while over-flagging certain human behavioral profiles (e.g., ENS-active DeFi users). Until agent detection generalizes cross-platform, complementary approaches---platform registries, voluntary disclosures, EIP-7702 delegation signals---are necessary supplements.
...
\begin{enumerate}
    ...
    \item \textbf{Single-class platforms confound the Level~5 result.} Because each platform category is either all-agent or all-human, LOPO simultaneously tests concept generalization (agent vs.\ human) and domain adaptation (familiar vs.\ novel behavioral profile). The pooled AUC of 0.42 is therefore an upper bound on the cross-platform penalty; a mixed-class LOPO would provide a tighter estimate. This is the most important structural limitation of the cascade's headline number.
...
The cascade from 0.98 to 0.42 is itself our main contribution. It reveals a gradient of evaluation realism, not a binary pass/fail for on-chain agent detection.

At each level, we stripped away an evaluation shortcut and lost performance:
...
\begin{itemize}
    ...
    \item \textbf{Level 3 $\to$ 5} (0.67 $\to$ 0.42): Holding out entire platforms removed the remaining signal and pushed pooled AUC below chance.
\end{itemize}

At Level~2, detection works: AUC~0.80 with precision~0.81 is a useful triage capability within the training distribution. At Level~5, it does not transfer: pooled AUC~0.42 across 14 held-out platform categories---though amplified by the single-class-per-platform confound---shows that the classifier has learned platform fingerprints rather than a universal agent signature. The best-recognized category (pilot agents, 80\% recall) succeeds because similar addresses exist in other training categories; the worst (DeFi high-frequency traders, 14\% recall) fails because its behavioral profile is unique. The research agenda is therefore not ``can we detect agents?'' (yes, within context) but ``can we build platform-invariant features?'' (not yet).
...
For the field to make progress, three things must change: (1)~cross-platform evaluation must become a reporting standard, with explicit acknowledgment of single-class-per-platform confounds when they exist; (2)~feature engineering must move toward platform-invariant representations; and (3)~the community must accept that honest AUCs will be lower than leaky ones---and that lower, honest numbers are the only ones worth building on. All code, features, labels, and diagnostic artifacts are released.
