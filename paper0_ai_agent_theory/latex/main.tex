...
\begin{document}
\title{Understanding the On-Chain AI Agent: A Four-Condition Definition and Eight-Category Taxonomy Validated on 2,744 Ethereum Addresses}
\author{NSF Project Team}
\affiliation{\institution{University}\country{USA}}

\begin{abstract}
We propose the first formal definition of on-chain AI agents (C1--C4) and validate an eight-category taxonomy on 2,744 Ethereum addresses.
Our most consequential finding is that LLM-powered agents are behaviorally indistinguishable from DeFi management agents at the transaction level (F1\,=\,0.44 across 23 and 31 features), implying that any regulatory framework based solely on transaction behavior cannot isolate LLM-driven activity.
Unsupervised clustering reveals that behavioral features saturate at three super-clusters regardless of method (K-Means silhouette peak at $k{=}3$; HDBSCAN finds only 2 density clusters), while the eight semantic categories require provenance metadata---the taxonomy captures conceptual distinctions that exist within a behaviorally homogeneous landscape.
Progressive validation achieves 95.3\% accuracy on the five majority classes and demonstrates cross-study feature transfer (Bridge F1: $0.31 \to 0.76$, DAO F1: $0.14 \to 0.43$ with independently developed AI-detection features).
A circularity ablation removing the four features shared between projection rules and classifier shows accuracy drops from 92\% to 87\%, but classes not defined by those features (LLM, Bridge) are unaffected.
\end{abstract}
...
\section{Understanding the On-Chain AI Agent: A Four-Condition Definition and Eight-Category Taxonomy Validated on 2,744 Ethereum Addresses}
...
\textbf{Key findings.} Three results anchor the paper. (1)~Unsupervised clustering shows behavioral silhouette is maximized at $k{=}3$, not $k{=}8$: the eight semantic categories compress into three super-clusters aligned with the C3/C4 definitional boundary, establishing that behavioral taxonomies saturate at three clusters. (2)~The five-class classifier on 23~features achieves 97.4\% accuracy and macro-F1 0.87; expanding to an eight-class, 31-feature classifier incorporating AI-detection features from our companion sybil study yields 95.3\% accuracy and macro-F1 0.69 with large gains on previously weak categories (Bridge F1: $0.31 \to 0.76$; DAO F1: $0.14 \to 0.43$). (3)~The LLM-Powered Agent class remains stubborn at F1 = 0.44 even with the expanded feature set---the on-chain behavioral overlap with DeFi Management Agents is not resolvable with transaction-level features alone, which is itself a finding with implications for governance and disclosure.
...
3. \textbf{Four-method progressive validation on 2,744 Ethereum addresses.} A rule-based taxonomy projection, a K-Means cluster sweep, a five-class supervised classifier (97.4\% accuracy, macro-F1 0.87), and an eight-class classifier with cross-study features (95.3\% accuracy, macro-F1 0.69) jointly stress-test the taxonomy at increasing granularity.

4. \textbf{Three honest findings about where the taxonomy breaks.} (a) The behavioral silhouette is maximized at $k{=}3$, not $k{=}8$, so the eight categories are semantically coherent but \textit{behaviorally over-split} at the current feature level. (b) Adding Paper~3's eight AI-detection features (31 total) lifts eight-class accuracy from 92.2\% to 95.3\%, with large gains on Bridge ($+0.45$) and DAO ($+0.29$). (c) The LLM-Powered Agent class remains at F1 = 0.44 even with 31 features---the behavioral overlap with DeFi Management Agent is not resolvable with transaction-level features alone, establishing a concrete boundary of chain-only observation.
...
The paper proceeds as follows. Section 2 reviews related definitions. Section 3 formalizes C1-C4 and argues for their non-redundancy. Section 4 presents the progressive on-chain validation, building from 3-class behavioral clusters through 5-class and 8-class classifiers to a 31-feature cross-study expansion. Section 5 discusses the three key findings. Section 6 treats limitations, and Section 7 concludes.
...
\subsection{5.1 The LLM-Powered Agent weak spot: a harder problem than predicted}

The single most important empirical finding of this paper is that the LLM-Powered Agent class remains the weak spot across \textit{both} feature scales tested. Under the 23-feature five-class classifier, LLM-Powered achieves F1 = 0.47. Under the expanded 31-feature eight-class classifier incorporating Paper~3's AI-detection features, LLM-Powered achieves F1 = 0.44---a slight \textit{regression}, not the improvement to 0.80+ that our earlier analysis predicted.

\textbf{What the 31-feature expansion fixes and what it does not.} The eight Paper~3 features produce dramatic improvements for three previously weak classes: Cross-Chain Bridge F1 jumps from 0.31 to 0.76, Autonomous DAO from 0.14 to 0.43, and RL Trading from 0.07 to 0.22. The feature \texttt{eip1559\_tip\_precision} alone accounts for 34\% of the 31-feature model's importance, and it is the feature most responsible for separating Bridge agents from the DeFi Management cluster. But LLM-Powered Agent does not benefit. Of 71 true LLM-Powered instances, 50 are still misclassified as DeFi Management Agent (70\% confusion rate, up from 60\% in the five-class setting). The expanded feature set helps the model distinguish \textit{other} agent subtypes but does not resolve the LLM/DeFi boundary.

\textbf{Why the prediction failed.} Our earlier hypothesis was that Paper~3's features---designed to separate AI-driven entities from humans---would also separate LLM-driven agents from rule-driven agents. This was wrong. The Paper~3 features capture the AI/human boundary (which is why Bridge and DAO classes improve: they are now separable from the DeFi Management majority). But the LLM-Powered vs.\ DeFi Management distinction is \textit{within} the AI-agent population, not across the AI/human boundary. From the perspective of all 31 features, an Autonolas rule engine and a prompt-driven Eliza agent produce nearly identical on-chain footprints: both are busy, both interact with multiple protocols, both have adaptive gas strategies, and both exhibit AI-like gas precision patterns.

\textbf{This is itself a finding.} LLM-Powered Agents are behaviorally indistinguishable from DeFi Management Agents using transaction-level features alone. The distinguishing features of LLM reasoning---context sensitivity, plan revision, prompt-driven action novelty, variable-length reasoning chains---do not project onto any of the 31 features. This negative result establishes a concrete boundary of transaction-level behavioral analysis: \textit{what an agent decides} (LLM vs.\ rule-based policy) is not observable from \textit{how it transacts}.

\textbf{Implications for future feature engineering.} Resolving the LLM-Powered weak spot likely requires features that capture LLM-specific behavioral signatures at a different abstraction level: prompt-latency patterns (LLM inference takes variable time depending on prompt complexity), context-window effects (action diversity should correlate with context length), temporal autocorrelation of action novelty (LLMs produce more semantically diverse action sequences than rule engines), and mempool submission timing jitter (LLM inference latency introduces characteristic timing noise). These are fundamentally different from the gas-precision and timing-regularity features that dominate the current 31-feature set.

\textbf{Governance implication.} A regulator or platform that wants to enforce disclosure on ``LLM-powered agents specifically''---for instance, because the governance risk profile of a prompt-driven agent is different from that of a rule-driven one---cannot do so with current 31-feature behavioral tooling. Any rule that says ``flag all LLM agents'' will either over-capture (catching DeFi management agents too) or under-capture (missing 70\% of LLM agents). This is a concrete and now \textit{empirically demonstrated} example of where feature engineering has practical governance implications.
...
Empirical validation on 2,744 Ethereum agent addresses yields five findings. First, unsupervised cluster validation reveals that behavioral silhouette is maximized at $k{=}3$, not $k{=}8$: the eight semantic categories compress into three behavioral super-clusters aligned with the C3/C4 definitional boundary. Second, a five-class classifier on 23~features reaches 97.4\% accuracy and macro-F1 0.87, with four of five classes exceeding F1 0.85. Third, expanding to all eight categories with 23~features drops accuracy to 92.2\%, exposing weak discrimination on Bridge, DAO, and RL classes. Fourth, adding Paper~3's eight AI-detection features (31~total) recovers accuracy to 95.3\% with large per-class gains (Bridge F1: $0.31 \to 0.76$; DAO F1: $0.14 \to 0.43$), validating cross-study feature transfer. Fifth, and most consequential, the LLM-Powered Agent class remains stubborn at F1 = 0.44 even with 31~features---the on-chain behavioral overlap with DeFi Management Agents is not resolvable with transaction-level features alone. This negative result establishes a concrete boundary of chain-only behavioral analysis and has direct implications for governance: any disclosure regime targeting LLM-powered agents specifically cannot rely on current feature engineering.
