...
\begin{abstract}
AI agents manage billions of dollars on decentralized-finance (DeFi) rails, yet ``AI agent'' still lacks a definition that maps to chain-observable behavior. We propose a four-condition definition---on-chain actuation (C1), environmental perception (C2), autonomous decision-making (C3), and adaptiveness (C4)---and an eight-category taxonomy spanning deterministic scripts through collaborative DAO agents. We validate both on 2,744 agent addresses using three complementary methods: a rule-based taxonomy projection, a K-Means cluster sweep, and gradient-boosting classifiers at two feature scales. Three key contributions emerge. First, unsupervised cluster validation reveals that behavioral silhouette is maximized at $k{=}3$, not $k{=}8$: the eight semantic categories compress into three behavioral super-clusters aligned with the C3/C4 boundary, establishing that \textit{behavioral taxonomies saturate at three clusters while semantic taxonomies require provenance}. Second, a five-class classifier on 23 features reaches 97.4\% accuracy (macro-F1 0.87), and an eight-class classifier on an expanded 31-feature set incorporating AI-detection features achieves 95.3\% accuracy (macro-F1 0.69), validating the taxonomy across granularity levels. Third, the LLM-Powered Agent class remains the empirical weak spot (F1 = 0.44 even with 31 features), behaviorally indistinguishable from DeFi Management Agents using transaction-level features alone---itself a finding with governance implications. We discuss implications for HCI, regulation, and the design of shared feature sets for downstream agent identification and Sybil-detection studies.
\end{abstract}
...
\subsection{4.11 Expert Validation via Modified Delphi Method}

The empirical validation in Sections 4.3--4.10 tests whether the taxonomy survives contact with on-chain data. A complementary question is whether it survives contact with \textit{expert judgment}. We designed and will execute a two-round modified Delphi study to assess whether domain experts endorse the C1--C4 definition and the eight-category taxonomy.

\subsubsection{4.11.1 Method}
...
\subsubsection{4.11.2 Analysis plan}
...
\subsubsection{4.11.5 Ethical review}
...
5. Execute the Delphi expert validation protocol (Section~4.11; materials finalized, pending IRB approval and recruitment).
