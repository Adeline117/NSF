...
\author[1]{Adeline Wen}
\affil[1]{Decentralized Computing Lab, University of Washington}
...
% FLUID: All numbers depend on experiment results (manifest.json)
% ---------------------------------------------------------------------------
\begin{abstract}
\noindent
...
$I(X;Y)$ and the label entropy $H(Y)$. Applied to an \nClasses-category taxonomy
spanning \nSamples{} blockchain addresses---a setting chosen because its public
transaction record allows exact measurement of $I(X;Y)$---we find a ceiling
of $\fanoAccuracyCeilingfeat$; the best supervised classifier reaches $\gbmAccuracyfeat$.
Table~\ref{tab:ceiling} reports the gap across feature sets: behavioral detection
is already near the information-theoretic limit. Unsupervised discovery saturates at
$\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$ clusters regardless of feature set (HDBSCAN: $\clusterHdbscanNClusters$; K-Means silhouette:
$\clusterKmeansOptimalSilhouetteK$). A mechanistic decomposition separates two signal channels: execution
speed is a \emph{tool} signature (LLM-driven DeFi operations: median
$\rtLlmDefiExecMedianS$\,s vs.\ ${\sim}\rtRuleDefiExecMedianS$\,s for rule-based agents; Cohen's $d \approx 0$,
not a reliable difference) while inter-decision interval is an
\emph{agent} signature (general reaction time: $\rtLlmMedianReactionS$\,s vs.\ ${\sim}\rtRuleMedianReactionS$\,s;
Cohen's $d=\rtCohensDLlmVsDefiMedian$). These findings imply that, on the Ethereum channel
...
% FLUID: corpus size, ceiling, classifier accuracy depend on manifest
...
$\nSamples$ real agent deployments, using a setting in which the complete
...
feature sets ($X_{\nFeaturesBase}$, $X_{\nFeaturesFull}$, $X_{47}$), the tightest achievable ceiling
is $\fanoAccuracyCeilingfeat$. Real classifiers trained on the same data reach
$\gbmAccuracyfeat$. \emph{The gap is $0.9$ percentage points
(verified across five classifier families; bootstrap 95\% CI on $I(X;Y)$:
$[1.38, 1.48]$ bits, $n{=}1000$): behavioral detection is already at the
...
$\nFeaturesBase$ to $47$ features raises the ceiling by less than one percentage point.

% FLUID: all reaction-time numbers, Cohen's d, cluster counts depend on manifest
...
than rule-based managers ($\rtLlmDefiExecMedianS$\,s vs.\ ${\sim}\rtRuleDefiExecMedianS$\,s), but this difference
...
($\rtLlmMedianReactionS$\,s vs.\ ${\sim}\rtRuleMedianReactionS$\,s; $d = \rtCohensDLlmVsDefiMedian$), reflecting language-model
...
K-Means and density-based clustering (HDBSCAN) saturate at $\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$ natural
behavioral clusters regardless of feature-set richness, even as the
taxonomy itself distinguishes \nClasses{}. Third, the ceiling is \emph{channel-specific}: a replication on the
...
% FLUID: headroom value depends on fano_headroom_pp_31feat
the head-room at $X_{\nFeaturesFull}$ is $\fanoHeadroomPpfeat$ percentage points---the smallest
...
% FLUID: corpus size, n_classes, feature-set dimensions depend on manifest
We estimate $I(X;Y)$ using a Kraskov-style $k$-nearest-neighbour plug-in
estimator\cite{kraskov2004estimating} applied to a corpus of $\nSamples$
...
of $K=\nClasses$ taxonomy categories by a provenance-anchored rule set, yielding
$H(Y) = 1.697$ bits. We evaluate three feature sets of increasing
expressiveness: $X_{\nFeaturesBase}$ (transaction-content features),
$X_{\nFeaturesFull}$ ($X_{\nFeaturesBase}$ augmented with cross-study AI-detection features from an
independent sybil study), and $X_{47}$ ($X_{\nFeaturesFull}$ augmented with
decision-process features capturing error recovery, session dynamics, nonce
regularity, and gas-price microstructure).

% FLUID: ceiling band and headroom values depend on manifest
...
band between $\fanoAccuracyCeilingfeatB$ and $\fanoAccuracyCeilingfeat$. Measured classifier accuracies sit
$\fanoHeadroomPpfeat$--$\fanoHeadroomPpfeatB$ percentage points below the ceiling
(Table~\ref{tab:ceiling}).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/fig1_mi_vs_accuracy.pdf}
  % FLUID: caption numbers depend on feature-set dimensions and headroom
  ...
  $I(X;Y)$, together with three measured feature sets ($X_{\nFeaturesBase}$, $X_{\nFeaturesFull}$,
  ...
  $X_{\nFeaturesFull}$ the head-room is $\fanoHeadroomPpfeat$ percentage points.}
  \label{fig:ceiling}
\end{figure}

% FLUID: entire table depends on MI estimation and classifier results
\begin{table}[t]
  \centering
  \small
  \begin{tabular}{lcccc}
    ...
    \midrule
    $X_{\nFeaturesBase}$ & $1.403$ & $0.294$ & \fanoAccuracyCeilingfeatB & \dpBaselinefeatAccuracy \\
    $X_{\nFeaturesFull}$ & \fanoMiJointBitsfeat & $0.259$ & $\mathbf{\fanoAccuracyCeilingfeat}$ & $\mathbf{\gbmAccuracyfeat}$ \\
    $X_{47}$ & $1.420$ & $0.276$ & $0.970$ & \dpfeatAccuracy \\
    \bottomrule
  \end{tabular}
  ...
  $N=\nSamples$ agent addresses. $H(Y) = 1.697$ bits; $K = \nClasses$ categories.
  A majority-class baseline (always predicting DeFi Management Agent)
  achieves $60.8\%$ accuracy; the classifier's $\gbmAccuracyfeat$ substantially
  exceeds this. Balanced accuracy (weighting each class equally) is $68.2\%$,
  confirming performance across minority classes; macro-F1 is $\gbmFMacrofeat$.
  The
...
% FLUID: tightness results [2026-04-20]
\paragraph{Tightness verification.}
A Fano bound is a \emph{lower} bound on error; reviewers may ask whether
it is loose. We verify tightness in two ways. First, we train five model
families---GBM, HistGradientBoosting, Random Forest, MLP (2 hidden
layers), and SVM-RBF---under 10-fold CV on the same $X_{\nFeaturesFull}$
data. No model exceeds GBM ($95.19\%$) by more than $0.04$ percentage
points (Table~\ref{tab:tightness}); the gap between the Fano ceiling
($96.1\%$, at $k{=}5$) and the best classifier is therefore $0.9$ pp.
Second, we bootstrap-resample $I(X;Y)$ ($n{=}1000$): mean $= 1.431$
bits, 95\% CI $= [1.382, 1.483]$, confirming the MI estimate is stable
(coefficient of variation $= 1.9\%$). The Cover--Hart\cite{cover2006}
$k$-NN Bayes-error estimate gives an upper bound on achievable accuracy
of $91.5\%$; the Fano ceiling ($96.1\%$) and the best classifier
($95.2\%$) both lie above this, consistent with the sandwich
Cover--Hart $\leq$ Bayes $\leq$ Fano.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    \toprule
    Bound / Estimate & Error & Accuracy \\
    \midrule
    Cover--Hart $k$-NN Bayes  & $0.085$ & $91.5\%$ \\
    Best observed (GBM, 10-fold CV) & $0.048$ & $95.2\%$ \\
    \textbf{Fano ceiling} ($k{=}5$) & $\mathbf{0.039}$ & $\mathbf{96.1\%}$ \\
    \bottomrule
  \end{tabular}
  \caption{Tightness verification. The Fano ceiling is only $0.9$ pp
  above the best observed classifier, confirming the bound constrains
  the problem (not just one model). Five classifier families
  (GBM $95.19\%$, HistGBM $95.15\%$, RF $94.57\%$, SVM $90.53\%$,
  MLP $89.65\%$) all fall below the ceiling. The Hellman--Raviv bound
  is uninformative here ($H(Y \mid X) < 1$ bit).}
  \label{tab:tightness}
\end{table}
...
ceiling ($\fanoAccuracyCeilingfeat$) and temporal accuracy ($\sim$67\%) is attributable to
...
% FLUID: per-class F1 values depend on classifier results
...
distribution of discriminability across categories.
Balanced accuracy ($68.2\%$; i.e., the unweighted mean of per-class recalls)
confirms that the classifier performs well beyond majority-class voting
($60.8\%$) but that minority-class performance drags the class-averaged metric
far below the headline overall accuracy.
Three of the \nClasses{}
...
classifier with the $X_{\nFeaturesFull}$ feature set; a fourth, MEV Searcher, reaches
...
that remains stubborn: its F1 rises from $0.38$ under $X_{\nFeaturesBase}$ to
$0.49$ when decision-process features are added, and never exceeds
$0.54$ in any configuration we tested. Under $X_{\nFeaturesFull}$, LLM-Powered precision is
...
% FLUID: cluster counts and silhouette values depend on manifest
...
analysis sweeping $k \in \{3, 4, \ldots, 15\}$ peaks at $k=\clusterKmeansOptimalSilhouetteK$
(silhouette $\ablationKmeansKSilhouette$---a value conventionally interpreted as weak-to-no
cluster structure, well below the $0.5$ threshold typically considered
evidence of meaningful separation\cite{kaufman1990finding}; the $k{=}\clusterKmeansOptimalSilhouetteK$
...
\texttt{min\_cluster\_size}$\in\{20,50\}$ identifies only $\clusterHdbscanNClusters$ density
clusters. A taxonomy with \nClasses{} semantically distinct classes therefore
dissolves, in behavior space, into $\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$ weakly separated
super-clusters (Figure~\ref{fig:clustering}). Supervised classifiers
\emph{can} learn the full \nClasses-way partition when labels are supplied, but
the structure is \emph{not discoverable} from behavior alone.

% FLUID: accuracy range and cluster range depend on manifest
Supervised accuracy (92--95\%) measures \emph{classifiability given labeled
training data}; unsupervised clustering ($\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$ clusters) measures
\emph{discoverability without labels}. These are distinct properties, and
their divergence is itself the central finding: the \nClasses{} categories are
...
% FLUID: cluster counts, silhouette, best-k depend on manifest
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/fig2_cluster_saturation.pdf}
  \caption{\textbf{Unsupervised clustering saturates at $\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$ groups
  regardless of feature-set expressiveness.} K-Means silhouette across $k
  \in \{3, \dots, 15\}$ peaks at $k = \clusterKmeansOptimalSilhouetteK$ ($\ablationKmeansKSilhouette$), with a secondary plateau
  at $k = \clusterBestK$. HDBSCAN (density-based, no pre-specified $k$) finds $\clusterHdbscanNClusters$
  density clusters. The taxonomy's \nClasses{} categories (colored markers in
  the inset PCA projection) do not separate in the unsupervised geometry.}
  \label{fig:clustering}
\end{figure}

% FLUID: MI values, headroom values depend on manifest
Second, \emph{adding features does not help.} Between $X_{\nFeaturesBase}$ and $X_{47}$,
...
slightly from $X_{\nFeaturesFull}$ ($\fanoMiJointBitsfeat$ bits) to $X_{47}$ ($1.420$ bits); this
$0.017$-bit drop is within the $\pm 0.03$-bit cross-validation tolerance
of the $k$-NN estimator (Methods). The slight MI decrease from $X_{\nFeaturesFull}$
(\fanoMiJointBitsfeat) to $X_{47}$ (1.420) is within the estimator's sampling noise
($\pm$0.8\,pp at $k{=}5$ neighbors); we do not interpret this as
meaningful.
The classifier-ceiling head-room shrinks from
$\fanoHeadroomPpfeatB$ to $1.66$ percentage points---but this shrinkage is driven almost
...
% FLUID: feature importance depends on manifest (top_feature_eip1559_tip_precision_importance)
...
joint $I(X_{\nFeaturesFull};Y)$, so additional features contribute diminishing
...
% FLUID: cross-chain accuracy depends on cross-chain experiment
...
% FLUID: evasion simulation results [2026-04-20]
\paragraph{Adversarial robustness.}
We simulate an LLM agent adding timing noise to mimic a DeFi Management
Agent by perturbing five timing features toward the DeFi class distribution
at noise level $\sigma \in \{0, 0.25, \ldots, 2.0\}$.
At $\sigma = 1.0$ (timing features drawn from the DeFi distribution),
Cohen's $d$ collapses from $1.76$ to $0.10$ (negligible), but the Fano
ceiling \emph{rises} from $97.2\%$ to $97.4\%$ and overall accuracy
improves to $95.7\%$. This paradox arises because evasion makes LLM
agents resemble the majority class (DeFi Management, $n = 1{,}669$),
improving population-level classification while rendering individual
LLM agents undetectable. At $\sigma = 2.0$, LLM-class F1 \emph{rises}
to $0.81$ because the added noise overshoots the DeFi distribution,
creating a new detectable signature.
The policy implication is sharp: the Fano ceiling bounds
\emph{category-level} classification accuracy, not \emph{individual-agent}
identification. An adversary who adjusts timing to blend into the majority
class evades detection without violating the information-theoretic bound.
Regulation that targets specific agent categories (e.g., LLM-driven)
cannot rely on behavioral observation alone, even though the ceiling
for the coarser question (``is this address automated?'') remains tight.
...
% FLUID: total DeFi txs depends on manifest (rt_total_defi_txs)
Across \rtTotalDefiTxs{} DeFi transactions in our corpus, we decompose inter-event
...
\begin{figure}[t]
  ...
  % FLUID: all reaction-time values, Cohen's d, n depend on manifest
  ...
  agents (blue, $\rtLlmDefiExecMedianS$\,s) vs.\ rule-based DeFi managers
  (orange, ${\sim}\rtRuleDefiExecMedianS$\,s)---Cohen's $d \approx 0$ ($p = 0.29$), not a
  statistically reliable difference at full scale ($n = \nSamples$).
  Right panel: the general inter-transaction interval
  is reliably \emph{slower} for LLM-driven agents ($\rtLlmMedianReactionS$\,s vs.\ ${\sim}\rtRuleMedianReactionS$\,s;
  $d=\rtCohensDLlmVsDefiMedian$, $p < 0.001$ at $n=\nSamples$), the largest effect size in the
...
At full scale ($n = \nSamples$) this specific feature dropped to
$d \approx 0$: the pilot result did not replicate. The general
inter-transaction reaction time, however, strengthened to $d = \rtCohensDLlmVsDefiMedian$---
...
% FLUID: LLM F1 gain depends on manifest (dp_llm_f1_gain)
...
F1 by $+\dpLlmFGain$ (absolute) while transaction-content features leave
...
\begin{figure}[t]
  ...
  % FLUID: cross-chain accuracy numbers depend on cross-chain experiment
...
% FLUID: accuracy, ceiling, headroom values depend on manifest
...
from $\dpBaselinefeatAccuracy$ to $\gbmAccuracyfeat$ and the Fano ceiling from $\fanoAccuracyCeilingfeatB$ to
$\fanoAccuracyCeilingfeat$---but the head-room closes from $\fanoHeadroomPpfeatB$ to $\fanoHeadroomPpfeat$ percentage
points, meaning the classifier benefits while the ceiling barely moves.

% FLUID: cluster count depends on manifest (cluster_hdbscan_n_clusters)
\paragraph{Density-based validation.} HDBSCAN corroborates K-Means: at
\texttt{min\_cluster\_size} of $20$ and $50$, it finds $\clusterHdbscanNClusters$ density
clusters, with $16$--$26\%$ noise---converging on the same $\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$-cluster
coarse structure.
...
ceiling $97.6\%$ vs.\ $\fanoAccuracyCeilingfeat$ on the full corpus). Head-room
...
% FLUID: ablation accuracy drop depends on manifest (ablation_accuracy_drop_23_to_19)
...
classifier. Accuracy falls from $\dpBaselinefeatAccuracy$ to $87.4\%$ and macro-F1 from
...
\paragraph{External label validation.} External label validation against Etherscan's independently maintained contract/EOA classification confirms 93.5\% consistency with our provenance labels (250 addresses queried, 502 API calls).
...
\emph{behavioral} enforcement of them is bounded: on an \nClasses-category
taxonomy with open data, the best possible classifier has a minimum error
probability of $\fanoErrorLowerBoundfeat$ (Table~\ref{tab:ceiling}), and the hardest category (LLM-driven
...
(iii) The ceiling is stated for a fixed taxonomy granularity of $K=\nClasses$;
...
learning strategy vaults, $n=25$), yielding $N=\nSamples$ addresses across
\nClasses{} categories. Features were extracted from public transaction logs via
...
these dimensions contains $100$ cells, but only \nClasses{} are populated by
deployed systems as of the observation period. These collapse into \nClasses{}
...
against a discretized histogram estimator on $K = 3$ and $K = \nClasses$
...
spaces\cite{gao2017estimating}. Bootstrap resampling ($n{=}1000$) yields
a 95\% CI of $[1.382, 1.483]$ bits on $I(X;Y)$ for $X_{\nFeaturesFull}$
(mean $= 1.431$ bits, SD $= 0.027$, CV $= 1.9\%$). The marginal entropy $H(Y)$ was
...
comparison in Supplementary Table~S1. % FLUID: ceiling and error bound depend on manifest
For $X_{\nFeaturesFull}$, the loose bound
yields an accuracy ceiling of $1.0$ (uninformative) while the tight
bound yields $\fanoAccuracyCeilingfeat$ ($P_e^\star = \fanoErrorLowerBoundfeat$); we always use the tight
...
of which corroborate the $\clusterHdbscanNClusters$--$\clusterKmeansOptimalSilhouetteK$-cluster saturation. These robustness
...
deviation. Both quantities were computed on the full $\nSamples$-address
corpus covering $\rtTotalDefiTxs$ DeFi-targeted transactions. The
...
features, added in $X_{\nFeaturesFull}$) captures response-latency variance,
...
$256$~GB RAM (wall-clock $\approx 14$~h for the full $\nSamples$-address
...
separate LLM-driven from rule-based agents with Cohen's $d = \rtCohensDLlmVsDefiMedian$
