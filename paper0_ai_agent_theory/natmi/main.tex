...
$216$\,s vs.\ $289$\,s for rule-based agents; Cohen's $d \approx 0$,
...
than rule-based managers ($216$\,s vs.\ $289$\,s), but this difference
...
($441$\,s vs.\ $321$\,s; $d = 1.01$), reflecting language-model
...
\begin{table}[t]
  ...
  The majority-class baseline (predicting DeFi Management Agent for all
  addresses) achieves $60.8\%$ accuracy; all classifiers substantially
  exceed this floor. The
...
(silhouette $0.151$---a value indicating very weak cluster structure,
well below the $0.5$ threshold typically considered evidence of
meaningful separation\cite{kaufman1990finding}); HDBSCAN with
...
of the $k$-NN estimator (Methods) and should be interpreted as estimator
noise rather than a genuine information loss.
...
single feature with the highest marginal information content is
$\texttt{eip1559\_tip\_precision}$ at $0.739$ nats ($1.066$ bits)---but
the joint $I(X_{31}; Y)$ is only $1.437$ bits, confirming very
substantial redundancy. (Note: per-feature MI is estimated in nats by
\texttt{sklearn}; joint MI is estimated in bits via our $k$-NN plug-in.
Conversions use $1 \text{ nat} = \log_2 e \approx 1.443 \text{ bits}$.) The second mechanism is the category-level information floor:
...
We argued in the Introduction that the Ethereum corpus is a test bed.
The Fano methodology is domain-general, but the specific ceiling we
report is a property of the Ethereum observation channel. Three lines
of evidence support the \emph{within-blockchain} robustness of the
saturation finding, though cross-domain generalization remains an open
empirical question.
...
estimated from the outside---but the same Fano inequality applies.
Whether the ceiling is more or less forgiving on such platforms is an
open empirical question that depends on the specific feature families
available; the Polygon cross-chain result ($22\%$ transfer accuracy)
demonstrates that even structurally similar domains require
feature-specific recalibration.
A conservative reading of our result is that the
$2$-percentage-point head-room we report characterizes the Ethereum
channel under the studied taxonomy, and that extrapolation to other
