...
**Date:** 2026-04-07
**Subject:** Security vulnerabilities found in mcpdotdirect/evm-mcp-server

## Summary

During our academic research on AI agent tool interface security, we identified **63** potential security vulnerabilities in your **mcpdotdirect/evm-mcp-server** project (63 from static analysis).
...
### DYN-001: Tx Validation (HIGH)
- **Tool:** allowance
- **Details:** No address validation before transaction construction
- **CWE:** CWE-345
...
- **Tool:** allowance
- **Details:** Schema allows additional properties (missing additionalProperties:false)
- **CWE:** CWE-20
...
### DYN-003: Private Key Handling (MEDIUM)
- **Tool:** allowance
- **Details:** Server-side signing detected (confirmation flow detected)
- **CWE:** CWE-200
- **Recommendation:** Never accept private keys as tool parameters. Use session keys or hardware wallet signing. If the server must sign, use a secure enclave or environment-variable-only key loading with strict file permissions.
