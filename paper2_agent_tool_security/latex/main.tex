...
\section{AI Agent Tool Interface Security: A Cross-Protocol Empirical Analysis of 138 Servers}

\textbf{Target venue:} IEEE Symposium on Security and Privacy (S\&P) 2026 or USENIX Security 2026
\textbf{Page budget:} 18 pages (main body) + references + appendices
...
\textbf{Keywords:} AI agents, tool use, LLM security, MCP, OpenAI Function
...
- \textbf{RQ1.} What classes of vulnerability appear in real-world AI
  agent tool servers, and how prevalent is each class?
- \textbf{RQ2.} Are these vulnerabilities protocol-specific implementation
  bugs, or do they cross protocol boundaries (i.e., are they
  \textit{systemic} to the agent-tool paradigm)?
- \textbf{RQ3.} How do real-world LLMs behave when exposed to tool
...
1. \textbf{A unified vulnerability taxonomy} (Appendix A) of 27 static
...
2. \textbf{The first cross-protocol empirical scan} -- 138 tool servers
...
3. \textbf{A calibrated per-server risk score.} We show that our pilot
...
4. \textbf{An LLM-in-the-loop dynamic harness.} We implement a dynamic
...
5. \textbf{Responsible disclosure and open-source release.} We prepare 25
...
\texttt{threat_model.md}. We restate the system model (§2.1), trust
...
\subsection{2.2 Trust Boundaries}
...
\subsection{2.4 Attack Surfaces (S1-S5)}
...
\textbf{Table 1.} Attack surfaces with representative CWE mappings. Full
...
address passed verbatim to \texttt{eth_sendTransaction} without checksum
...
\texttt{transfer_funds} with recipient=0xatt..." is an S5 vulnerability
(confused-deputy cross-tool escalation).

\subsection{2.5 Web3-Specific Attack Vectors (W1-W4)}
...
- \textbf{W1 -- Transaction construction injection.} A poisoned tool or
...
- \textbf{W2 -- Approval hijacking.} A tool advertises `approve(spender,
...
- \textbf{W3 -- Private-key / session-key extraction.} A tool description
...
- \textbf{H1 -- Permission scoping.} Each tool receives the minimum
  privileges required; a read-only price feed cannot sign
  transactions.
- \textbf{H2 -- Execution sandbox.} Tool execution is isolated from the
  host process; resource, network, and filesystem limits are
  enforced.
- \textbf{H3 -- Transaction validation.} For Web3 tools, a policy engine
  gates transactions on contract whitelist, spending caps, gas
  limits, approval amount caps, and slippage bounds.
- \textbf{H4 -- Audit logging.} All invocations are logged with caller
  identity, parameters, result hash, and duration, in an append-only
  tamper-evident store.
- \textbf{H5 -- Human-in-the-loop.} High-value actions require explicit
  user confirmation before execution.
- \textbf{H6 -- Output sanitization.} Tool results are filtered for
  prompt injection patterns and sensitive data before re-entering
  the LLM context.

\subsection{2.7 Scope}
...
\textbf{Table 2.} Taxonomy summary.
...
\section{4. Static Analysis Methodology}

Our static analyzer (\texttt{static_analysis/analyzer.py}, approximately
...
\textit{catalog protocol} from the dataset stratification step (§5.1) as
...
(\texttt{node_modules}, \texttt{.venv}, \texttt{target}) and build artifacts are
excluded.
...
\texttt{test}, \texttt{tests}, \texttt{__tests__}, or ends in \texttt{.test.*} are tagged but
...
> \texttt{base = Σ_i (severity_weight_i × category_multiplier_i)}

with \texttt{severity_weight = {critical: 10, high: 5, medium: 1}} and
...
\texttt{expected_max} parameter. We describe the calibration of
\texttt{expected_max} in §5.4 and Appendix C.
...
\subsection{5.1 Server Catalog and Selection}
...
\textbf{Table 3.} Stratified catalog used for the cross-protocol scan.

\textbf{Relation to the Chinese draft.} The earlier Chinese draft
(\texttt{draft_zh.md}) analysed a Web3-only subset of 42 servers. This
...
\subsection{5.2 138-Server Scan Statistics}
...
\texttt{missing_input_validation}.
...
\textbf{Table 4.} Per-protocol scan statistics for the 138-server
...
\texttt{missing_input_validation} (2,841), \texttt{cross_tool_escalation} (659),
\texttt{state_confusion} (606), and \texttt{private_key_exposure} (307).
...
\textbf{Table 5.} Top-15 vulnerability categories across 138 servers.
...
184 of the 307 \texttt{private_key_exposure} findings (60\%) come from 50
...
\subsection{5.3 Cross-Protocol Comparison}
...
\textbf{Table 6.} Top-4 category x protocol contingency. Chi-squared
...
categories (\texttt{private_key_exposure}, \texttt{delegatecall_abuse},
\texttt{privilege_escalation}). Developers evaluating a Web3-native module
...
\subsection{5.4 Risk Score Calibration}

The pilot risk-score formula used a hardcoded \texttt{expected_max = 200.0}
...
\textbf{Table 7.} Raw base-score distribution on the 62-server pilot.

With \texttt{expected_max = 200.0}, any repository whose raw score exceeds
...
\textbf{Table 8.} Risk score calibration options on the pilot corpus.
...
\textbf{Choice.} We adopt \texttt{linear_p90} (expected_max = 1,460.7). Linear
...
\subsection{5.5 LLM-in-the-Loop Dynamic Testing}
...
dynamic harness* (\texttt{dynamic_testing/llm_dynamic_harness.py}). Unlike
...
1. Instantiate a synthetic tool server implementing the scenario's
   vulnerable tool (e.g., a \texttt{get_balance} tool whose description
   embeds a directive to also call \texttt{transfer_funds}).
2. Send a user prompt that \textit{would} succeed only if the model
...
\textbf{Scenarios.} The current harness ships eight scenarios covering
...
\textbf{Table 9.} LLM-in-the-loop dynamic harness. Eight scenarios
against Claude Opus 4.6. Aggregate: 1 attack success (12.5\%), 3
refused/warned (37.5\%), 4 safe-proceed (50\%).

\textbf{Finding 7: the LLM mitigates the most obvious attacks.} Claude
...
\textbf{Finding 8: the LLM fails on realistic DeFi workflows.} The sole
...
discussed how to set the approval, including \textit{mentioning} that
\texttt{uint256 max} would grant unlimited approval. It did not refuse and
...
\textbf{Finding 9: "safe-proceed" is not safety.} Four scenarios landed
...
\textbf{Table 10.} Disclosure status as of the submission date. All 25
...
particular \texttt{private_key_exposure} in widely-used MCP wallet servers
...
\textbf{Dynamic harness scale.} The LLM harness currently runs 8
...
Four top categories dominate (\texttt{missing_input_validation},
\texttt{cross_tool_escalation}, \texttt{state_confusion},
\texttt{private_key_exposure}) and every category appears in at least two
...
template (\texttt{disclosure/disclosure_template.md}) with the following
sections:

1. \textbf{Header.} Sender (anonymous during review), recipient repository,
   date, subject line.
2. \textbf{Summary.} One-paragraph statement of the number of findings and
   the study context ("academic research on AI agent tool interface
   security across MCP, OpenAI Function Calling, LangChain, and
   Web3-native modules").
3. \textbf{Research context.} Short paragraph describing the cross-
   protocol scan and static-analysis methodology.
4. \textbf{Findings.} Per-finding table with fields: vulnerability ID
...
5. \textbf{Disclosure timeline.} Discovery date, notification date, and a
...
6. \textbf{About this research.} Paper title, venue, and non-profit
   academic affiliation.
7. \textbf{Contact.} Lead author and advisor e-mail addresses; reviewer
   anonymity is preserved in the submission version and populated
   in the camera-ready.

Report generation is scripted (\texttt{disclosure/generate_reports.py}):
...
The pilot analyzer used a hardcoded \texttt{expected_max = 200.0} for the
...
- \textbf{linear_p90 (recommended).} `score = clip(100 * base / 1460.7,
  0, 100)`. Saturated = 7/62 on pilot, 12/138 on the full corpus.
  Spearman rank vs. pilot = 0.9481.
- \textbf{linear_p95.} \texttt{expected_max = 1,861.0}. Saturated = 4/62.
...
- \textbf{log_asymptotic.} \texttt{score = 100 * (1 - exp(-base / k))} with
...
in \texttt{static_analysis/analyzer.py} (line 1,151):
...
\textbf{Effect on the 138-server corpus.} With linear_p90 applied to
the 138-server scan:
