...
\texttt{threat\_model.md}. We restate the system model (\S2.1), trust
boundaries (\S2.2), attacker model (\S2.3), the five attack surfaces
(\S2.4), four Web3-specific attack vectors (\S2.5), and six harness
requirements (\S2.6).
...
(\texttt{draft\_zh.md}) analysed a Web3-only subset of 42 servers. This
...
specialisation for Web3 analysis (\S5.4).
...
catalog. Mean risk uses the recalibrated linear\_p90 scheme (\S5.4).
...
\midrule
linear (pilot) & expected\_max = 200 & 29/62 & 60.57 & 72.0 & 41.71 & --- \\
linear\_p90 & expected\_max = 1,460.7 & 7/62 & 25.54 & 9.86 & 32.47 & 0.9481 \\
linear\_p95 & expected\_max = 1,861.0 & 4/62 & 21.54 & 7.74 & 29.18 & 0.9476 \\
log\_asymptotic & k = 144.0 & 12/62 & 57.83 & 62.89 & 38.25 & 0.9475 \\
\bottomrule
\end{tabular}
\end{table}
...
- \textbf{linear\_p90 (recommended).} `score = clip(100 * base / 1460.7,
  0, 100)`. Saturated = 7/62 on pilot, 12/138 on the full corpus.
  Spearman rank vs. pilot = 0.9481.
- \textbf{linear\_p95.} \texttt{expected\_max = 1,861.0}. Saturated = 4/62.
...
- \textbf{log\_asymptotic.} \texttt{score = 100 * (1 - exp(-base / k))} with
